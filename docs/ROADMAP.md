...
**完了基準**: フィルタUI改善、グリッドビュー高視認性化、UIポリッシュ、交通ページ強化
...
**M4 成果物 (2026-04-03):**
- りんかい線・ゆりかもめ・都バス・BRT のロゴ画像追加（Wikimedia Commons / Public Domain）
- ヘッダー濃紺（slate-800/900/700）→ 薄グレー（slate-50/100/200）
...
### Phase 10〜14: マネタイズフェーズ 🚧 着手予定

**戦略の転換 (2026-05-25)**: 機能拡張だけではゴールに達しない。HK の最終ゴール「**誰かの役に立つと言える状態**」の最も嘘がつけない指標 = **収益が発生している** に再設定。フェーズを集客→計測→換金→品質向上の順に並べ替える。

**ファネル全体像**:

```
X 自動告知 → サイト流入 → 計測 → アフィリエイトクリック → 収益
                              ↑                     ↑
                  (混雑度チューニングで滞在向上)   (マップで差別化)
```

---

### Phase 10: アクセス計測導入 🗓 着手予定

**完了基準**: GA4 (or Vercel Analytics) 導入、チケットボタンCTR・流入経路・滞在時間が計測可能な状態

| # | マイルストーン | 状態 |
|---|---|---|
| M1 | 計測ツール選定 (GA4 / Vercel Analytics / Plausible) | 未着手 |
| M2 | サイトに計測タグ埋め込み + イベント発火 (チケットボタンクリック等) | 未着手 |
| M3 | ダッシュボード初期構築 (流入・滞在・CTR の3指標を見える化) | 未着手 |

**なぜ最優先**: マネタイズ施策 (アフィリエイト/X告知) の効果測定は計測なしには不可能。先に入れないと後続フェーズの意思決定が「勘」になる。

---

### Phase 11: X自動告知ボット 🗓 着手予定

**完了基準**: 新規 X アカウントから検出→1週間前→前日→当日の4段階で自動投稿、引用RTスレッド化、Free tier 枠内運用

| # | マイルストーン | 状態 |
|---|---|---|
| M1 | 新規X アカウント `@ariake_events` (or 別名) 作成 + Developer Portal で Free tier API キー発行 | HK本人作業 |
| M2 | `packages/x-poster/` 雛形実装 (Mock client) + テスト | 未着手 |
| M3 | テンプレ4種 (detected / 1week / 1day / 0day) + 投稿状態管理 (posted_tweets.json) | 未着手 |
| M4 | Live client 接続 + GitHub Actions 統合 (scrape.yml 後段に追加) | 未着手 |
| M5 | 初回数日間のドライラン → 本番運用切替 | 未着手 |

**MLB プロジェクト流用**: `src/mlb_analysis/posters/twitter_client.py` の Protocol/Mock/Live パターンを TypeScript で踏襲。npm パッケージは `twitter-api-v2` を使用 (引用RT対応)。

**API コスト**: $0/月 (Free tier の 1,500投稿/月で十分。推定使用量 120〜320投稿/月)。

**詳細設計**: `docs/X_POSTER_DESIGN.md`
**HK 本人作業手順**: `docs/X_API_SETUP_GUIDE.md`

---

### Phase 12: アフィリエイト統合 🗓 着手予定

**完了基準**: 既存チケットボタンに UTM パラメータ + アフィリエイト ID を付与、クリック→収益の経路確立

| # | マイルストーン | 状態 |
|---|---|---|
| M1 | アフィリエイトプログラム加入申請 (バリューコマース等経由でぴあ/e+/ローチケ) | HK本人作業 |
| M2 | 審査通過後、各社アフィID 受領 | HK本人作業 |
| M3 | `ticketPlatforms.ts` の URL ビルダーに UTM + アフィID 注入 | 未着手 |
| M4 | Phase 10 の計測ツールでクリック→アフィ遷移の経路を追跡 | 未着手 |

**注**: 審査期間が読めない (1日〜数週間)。M1 を Phase 11 と並行で着手すれば待ち時間を最小化できる。

---

### Phase 13: 混雑度チューニング 🗓 着手予定

**完了基準**: 現行の混雑度スコアと Google Maps Popular Times 実測値の相関 r > 0.7

| # | マイルストーン | 状態 |
|---|---|---|
| M1 | 検証データ収集 (Google Maps Popular Times を週次サンプリング) | 未着手 |
| M2 | サイトスコア vs 実測の相関分析 | 未着手 |
| M3 | アルゴリズム修正 (CATEGORY_MULTIPLIER / capacity / dayTypeBonus の係数調整) | 未着手 |
| M4 | 検証 → 本番反映 | 未着手 |

**位置づけ**: 数字の信頼性が上がる → 滞在時間・再訪率上昇 → 広告/アフィ単価上昇。Phase 10 (計測) があるため効果測定が可能になる。

---

### Phase 14: 混雑度マップ可視化 🗓 着手予定

**完了基準**: 有明エリアのヒートマップで日別混雑予測を可視化、Google Maps にない独自表現

| # | マイルストーン | 状態 |
|---|---|---|
| M1 | UI 設計 (HTML プロトタイプ複数案 → HK 選択) | 未着手 |
| M2 | Leaflet ベース実装 (既存 MapView 拡張) | 未着手 |
| M3 | SEO/SNS 拡散用の OG 画像自動生成 (日別) | 未着手 |

**差別化価値**: Google Maps Popular Times は施設単位・過去パターンのみ。本機能は「未来予測 × エリア統合」を視覚化する。

---

### Phase 9: チケット導線 直行リンク化 (B案) 🗓 候補・未着手

**動機**: A案 (Phase 8 M6) で検索URLフォールバックを実装したが、実検証でヒット率が低いことが判明（販売前・プラットフォーム独占の影響）。施設詳細ページから販売元URLを直接スクレイプして直行リンク化する。

| # | マイルストーン | 状態 |
|---|---|---|
| M1 | 各施設詳細ページの販売元リンク確認スパイク（GO/NO-GO 判定） | 未着手 |
| M2 | EventItem に ticketLinks フィールド追加 | 未着手 |
| M3 | スクレイパー拡張（施設ごとに段階的に） | 未着手 |
| M4 | TicketModal 改修（直行 + フォールバック） | 未着手 |
| M5 | テスト + コミット + 再 scrape | 未着手 |

**前提条件**: M1 スパイクで各施設詳細ページに販売元リンクが実在するか確認が必須。フィクスチャ確認では 0 ヒットだった。

**詳細プラン**: `~/.claude/plans/ok-b-foamy-cookie.md` および `memory/project_phase9_ticket_b_scraper.md`
