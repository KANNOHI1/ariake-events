...
export default function DayView({ events, onResetFilters, viewMode }: Props) {
  ...
  const todayBadgeClass =
    'text-xs font-medium text-primary-500 bg-[#fff3ed] px-2 py-0.5 rounded-full'
    ...
    <div>
      ...
      <div className="flex items-center justify-between border-b border-slate-100 bg-white px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {isToday ? (
            <span className={todayBadgeClass} aria-label="今日">
              今日
            </span>
          ) : (
            <button
              onClick={goToToday}
              className={`${todayBadgeClass} hover:bg-orange-100 transition-colors cursor-pointer`}
              aria-label="今日に戻る"
            >
              ↩ 今日に戻る
            </button>
      ...
      {dayEvents.length === 0 ? (
        <div className="text-center py-16 text-slate-500">
          ...
          <button onClick={onResetFilters} className="text-sm text-primary-500 underline cursor-pointer">
            絞り込みをリセット
          </button>
        </div>
      ) : (
