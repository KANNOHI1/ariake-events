...
- **Phase 8 M7** (完了 2026-04-05): FilterBar デスクトップPopover実装
  - lg以上: ボタン直下にPopoverパネル（施設+カテゴリチップ）、外クリックで閉じる
  - lg未満: 従来のBottomSheet維持
  - FilterBar.tsx: isOpen 1つで管理、Popoverパネル追加
...
- **カードの並び順改善** (完了 2026-04-07): sortEvents ユーティリティ追加・DayView/MonthView/CalendarView に適用
  - カテゴリ優先度: music(0)〜other(5)、4日以上開催 +10 ペナルティ
  - Date.UTC でタイムゾーン安全な日数計算
  - テスト: 7件追加（18スイート 150/150 PASS）、push済み
